---
categories: [TIL]
tags: [til, linux, deep learning, python]
...
- 미분하면 `0`일때는 0, 그 외에는 `1`이기 때문에 `기울기 소실` 발생시키지 않음
- `음수값`이라면?
  - 음수가 나오면 해당 노드는 학습 안됨
...
- 2차원 데이터를 1차원 데이터로 변환 (DNN 층의 입력이 1차원 이므로)
- 데이터 값의 범위를 0-255에서 0-1로 변환 (정규화(Scaling)) -> 과적합 줄어듬
