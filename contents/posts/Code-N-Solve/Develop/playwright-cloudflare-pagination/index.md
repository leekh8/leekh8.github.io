...
```python
page.evaluate("loadCommentList(event_id, 2, {isQuestion: 'Y'})")
```
...
```python
with page.expect_response("**/comment/comment_list**") as resp_info:
...
사이트의 JS를 분석하니 페이지 로드 완료 시 자동으로 `loadData(resource_id, 1, params)` 같은 함수가 호출됐다. 1페이지를 자동으로 불러오는 함수다.

이 함수가 호출되는 시점을 가로채서 `1` 대신 원하는 페이지 번호를 넣으면 된다. `page.add_init_script()`가 이 역할을 한다.
...
`page.add_init_script()`는 페이지의 모든 JS보다 먼저 실행된다. 사이트 JS가 로드되기 전에 `loadCommentList`를 감시하고 있다가, 사이트 JS가 이 함수를 정의하는 순간 가로채서 래핑한다.

결국 사이트 입장에서는 자기가 1페이지를 요청하는 것처럼 보이지만, 실제로는 우리가 원하는 페이지 번호가 들어간다. 그리고 새 컨텍스트에서 실행되니 Cloudflare도 정상 브라우저 접속으로 인식한다.
