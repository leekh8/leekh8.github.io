...
graph LR
    subgraph 송신측
        S7["7계층<br/>데이터"] -->|"HTTP 헤더 추가"| S4["4계층<br/>TCP 헤더 + 데이터"]
        S4 -->|"IP 헤더 추가"| S3["3계층<br/>IP 헤더 + TCP + 데이터"]
        S3 -->|"MAC 헤더 추가"| S2["2계층<br/>MAC + IP + TCP + 데이터"]
        S2 -->|"전기 신호 변환"| S1["1계층<br/>비트 스트림"]
    end

    subgraph 수신측
        R1["1계층<br/>비트 스트림"] -->|"신호 해석"| R2["2계층<br/>MAC 헤더 제거"]
        R2 -->|"IP 패킷 전달"| R3["3계층<br/>IP 헤더 제거"]
        R3 -->|"세그먼트 전달"| R4["4계층<br/>TCP 헤더 제거"]
        R4 -->|"데이터 전달"| R7["7계층<br/>데이터"]
    end

    S1 -->|"물리 전송"| R1

    style S7 fill:#3498db,color:#fff,stroke:#2980b9
    style S4 fill:#e67e22,color:#fff,stroke:#d35400
    style S3 fill:#e74c3c,color:#fff,stroke:#c0392b
    style S2 fill:#27ae60,color:#fff,stroke:#219a52
    style S1 fill:#7f8c8d,color:#fff,stroke:#6c7a7d
    style R1 fill:#7f8c8d,color:#fff,stroke:#6c7a7d
    style R2 fill:#27ae60,color:#fff,stroke:#219a52
    style R3 fill:#e74c3c,color:#fff,stroke:#c0392b
    style R4 fill:#e67e22,color:#fff,stroke:#d35400
    style R7 fill:#3498db,color:#fff,stroke:#2980b9
```
